...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!user && mounted) router.replace('/login');
    })();
  ...
  async function submit(e: FormEvent) {
    ...
    if (cleanName.length < 2) { setError('Brand or creator name must be at least 2 characters.'); return; }
    if (cleanSlug.length < 3) { setError('Choose a username with at least 3 characters.'); return; }

    setSaving(true);
    const supabase = createClient();
    const rpc = supabase.rpc('create_workspace', { workspace_name: cleanName, workspace_slug: cleanSlug, workspace_description: description.trim() || null });
    const timeout = new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Workspace creation timed out. Please try again.')), 12000));
    try {
      const { error: rpcError } = await Promise.race([rpc, timeout]);
      if (rpcError) { setError(rpcError.message); setSaving(false); return; }
      router.replace('/dashboard');
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unable to create your storefront. Please try again.');
      setSaving(false);
    }
  }
  ...
  async function checkUsername() {
    ...
    const supabase = createClient();
    const { data, error: checkError } = await supabase.from('organizations').select('id').eq('slug', slug).maybeSingle();
    setChecking(false);
    if (checkError) setError('Could not check this username right now.');
    else if (data) setError('That username is already taken.');
  ...
  return <main className={styles['onboarding-page']}><div className={styles['onboarding-card']}>
    ...
    <form onSubmit={submit}>
      ...
      <button type="submit" className={`primary ${styles['onboarding-submit']}`} disabled={saving}>{saving?'Creating your storefront…':'Continue'} <ArrowRight size={17}/></button>
