  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { data: member, error: memberError } = await supabase.from('organization_members').select('organization_id').eq('user_id', auth.user.id).limit(1).maybeSingle();
        if (memberError) throw memberError;
        if (!member?.organization_id) { setError('Ainda não tens uma organização configurada.'); return; }
        const [{ data: organization, error: orgError }, { data: items, error: productsError }, { data: shop, error: storefrontError }] = await Promise.all([
        ...
        const { data: analytics, error: analyticsError } = await supabase.from('analytics_events').select('*').eq('organization_id', member.organization_id).gte('created_at', since).order('created_at', { ascending: true });
