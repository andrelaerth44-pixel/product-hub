...
export default async function Storefront({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{category?:string}>}){
 ...
 {rest.length>0&&<section className={styles.grid}>{restWithImages.map(({item,image})=>{const price=money(item);return <article className={styles.card} key={item.id}>{image?<Link className={styles.cardImage} href={`/store/${store.organization.slug}/${item.slug}`}><img src={image} alt={item.name}/></Link>:<Link className={`${styles.cardImage} ${styles.noImage}`} href={`/store/${store.organization.slug}/${item.slug}`}>Product Hub</Link>}<div className={styles.cardBody}>{item.category&&<span className={styles.label}>{item.category}</span>}<h3 className={styles.cardTitle}>{item.name}</h3>{item.description&&<p className={styles.cardDescription}>{item.description}</p>}<div className={styles.cardFooter}><TrackedPurchaseLink organizationId={store.organization.id} productId={item.id} href={item.purchase_url} className={styles.cardBuy}>Comprar <ArrowRight size={14}/></TrackedPurchaseLink>{showPrices&&price&&<strong className={styles.cardPrice}>{price}</strong>}</div></div></article>})}</section>}
 {!products.length&&<div className={styles.empty}><h2>Nenhum produto publicado.</h2><p>Esta categoria ainda não tem produtos disponíveis.</p></div>}<footer className={styles.footer}><span>Product Hub</span><strong>{store.organization.name}</strong></footer></div></main>
}
